...
//        val ret = HidDevice!!.sendReport(, report.ReportId.toInt(), report.ReportData)
//        if (!ret) {
...
//        }
    ...
    fun leftBtnClick() {
        leftBtnDown()
        Utilcls.UtilCls.DelayTask({ leftBtnUp() }, 20, true)
    }

    fun leftBtnClickAsync(delay: Int): TimerTask {
        return Utilcls.UtilCls.DelayTask({ leftBtnClick() }, delay, true)
    }
